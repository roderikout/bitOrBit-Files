...
function printTexts()
  ...
  printText("Level: " .. tostring(level.currentLevel), 10, yPos * 3)
  local font = love.graphics.setNewFont(15)
  printText("Press 'i' for keys", 10, yPos * 9)
 -- printText("Orbits needed to win: " .. tostring(#level.orbitsNeededToWin), 10, yPos * 12)
  --for i, prob in ipairs(probes) do
    --[[for j, frag in ipairs(fragmentsOfProbe) do
    printText("Prob #: " .. tostring(j) ..
              ", Nombre: " .. tostring(frag.number)
              --", Selected: " .. tostring(prob.selected) ..
             -- ", Die: " .. tostring(prob.die)
              , 10, 140 + (j * 10))
    end--]]
  --end
  if level.instructionsOn then
    --setColor(0,0,0)
