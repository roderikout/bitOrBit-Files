...
function Probe:checkLowHigh(planet)  -- saber el punto mas alto y mas bajo de la orbita de la probe
  self.intersection = false
  self.vectorPlanet = vector.new(planet.x, planet.y)
  self.zonaMin = planet.colorZones[self.number].min
  self.zonaMax = planet.colorZones[self.number].max 
  self.distancia = utils.distanceTo(self.x, self.y, planet.x, planet.y)
  self.angle = math.atan2((self.pos.y - planet.y), (self.pos.x - planet.x))

  self:checkInsideOrbit(planet.pos)
  self:checkLaps()
end
...
function Probe:checkInsideOrbit(planetPosition) --para saber si la probe esta dentro de su orbita
   if self.distancia + self.r > self.zonaMin and self.distancia + self.r < self.zonaMax and #self.probeMinMax == 0 then 
    ...
    elseif self.distancia + self.r < self.zonaMin or self.distancia + self.r > self.zonaMax then
      self.probeInOrbitLine = {}
      self.probeMinMax = {}
      self.laps = 0
      self.inMyOrbit = false
...
function Probe:checkLaps() --Viene de CheckInsideOrbit y Luego a Game.checkOrbitsDone, cuenta las vueltas que da sin salirse de la orbita
  if self.inMyOrbit then
    self.intersection = utils.intersectSegment(self.prevPos.x, self.prevPos.y, self.postPos.x, self.postPos.y, self.probeInOrbitLine[1].pMin.x, self.probeInOrbitLine[1].pMin.y, self.probeInOrbitLine[1].pMax.x, self.probeInOrbitLine[1].pMax.y)
  end
...
function Probe:drawProbeInOrbitLine()  --revisar qué pasa con esta funcion
  if #self.probeInOrbitLine > 0 then --and self.drawInOrbitLine then
    love.graphics.setColor(255,255,255,255)
