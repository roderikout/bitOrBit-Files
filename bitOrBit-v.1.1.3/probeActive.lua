...
function ProbeActive:checkInsideOrbit() --chequea que una probe se encuentra en su orbita, (revisar actualizacion de zonaMin y zonaMax para cuando choca una probe con el planeta)
...
-- new
function ProbeActive:drawArcProbeInsideOrbit()
  for i, planet in ipairs(game.level.planetas) do
    if self.inMyOrbit then
      --if #self.probeInOrbitLine > 0 then
        --if self.probeInOrbitLine[1] then
          local angIn = self.probeInOrbitLine[1].pIn:toPolar()
          --love.graphics.arc("line","open", planet.x, planet.y, 10, 100, angle)
          love.graphics.arc("line","open", planet.x, planet.y, 100 * self.number, angIn.x, angle)
       -- end
      --end
      --love.graphics.setLineWidth(1)
    end
